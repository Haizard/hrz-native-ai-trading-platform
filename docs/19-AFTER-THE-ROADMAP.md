...
| 20 | **The footprint's legibility rule is stated once and implemented in three places** | `frontend/chart-engine/src/footprint.rs` holds `MIN_FONT_PX = 7.0`, `legible_rows()` and `font_for_width()`; `frontend/app/app.js` holds `MIN_COLUMN_PX = 64`; and `tools/footprint_preview.mjs` is a line-for-line copy of `drawFootprintGrid`. Added 2026-09-17 | `docs/14` now says the two halves of the font decision live in one place, in the engine — and that is true of `show_text`. It is **not** true of the column count the shell *asks for*: `MIN_COLUMN_PX = 64` was chosen by hand arithmetic against the engine's `MIN_FONT_PX = 7.0` and a 0.62 glyph ratio, and nothing fails if either number moves. This is the original defect's own shape — a threshold in the shell and a clamp in the engine, drifting apart until a real window drew no numbers — left in place one level up, and it is exactly what a "fixed it" that is not tested for *staying* fixed looks like | The column count the shell requests is derived from the engine rather than hand-computed (a function the shell calls, or a figure the engine reports), and either the preview renders through the real `app.js` renderer or a check fails when the copy diverges from it |
| 21 | **The gateway's live feed is never written to the database, so every stored series is frozen at the last backfill** | `insert_candles` and `insert_trades` have exactly one caller between them: `tools/xtask/src/main.rs`. `crates/market-data` has no `db` edge by design, and `crates/api-gateway/src/bots.rs`'s `run_binance_feed` subscribes the collector's trades and depth but never drains `collector.candle_stream()` or `collector.trade_stream()` anywhere. Measured on the running deployment 2026-09-17: the newest stored BTCUSDT 1m candle was **13.1 hours** behind the clock (`open_time=1789632240` against `1789679460`) and `GET /footprint/coverage` stopped at the same second (`to=1789632345379`) | Row 12 closed "the live collector never persisted candles" — and it is **still true of the gateway**, which is where the live path actually runs. The live side reads as live: `BinanceCollector` publishes closed candles into the bus, `/ws/market` fans them out, `MD_FEED_AGE` ages, and an open chart appends them to its in-memory series. So a chart in **candles** mode moves while it is open and loses every one of those bars on reload, and a chart in **footprint** mode does not move at all, because `/footprint` is built from stored trades. That is the user-visible report — "it has been over 7 hours and I am seeing the same candles on each timeframe" — and it is the organising rule in its purest form: the collector publishes, the socket fans out, the metric ages, and **nobody writes** | A pump in `api-gateway` (or an owner chosen deliberately) drains the collector's candle and trade streams into `candles`/`trades` through the idempotent upserts that already exist, so the stored series tracks the clock and a reload shows what the chart was showing. `tools/xtask/src/main.rs`'s `pump_candles`/`pump_trades` are the precedent and should be **shared** rather than copied — a second copy of a batching writer is the row-20 defect again. The trade half is bounded by `docs/13`'s retention/downsampling (row 3), which lands with it or the write is ~10 rows/s on one symbol forever |
